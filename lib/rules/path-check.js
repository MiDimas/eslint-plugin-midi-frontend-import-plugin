    ...
    return {
      ...
      ImportDeclaration(node) {
        if(node.source){
          ...
          let fromFilename = context.physicalFilename;
          if(!fromFilename) {
...
function shouldBeRelative(from, to) {
  ...
  const arrNorm = normalizedPath.split('src')
  const projectFrom = arrNorm.length>1 ? arrNorm[1] : arrNorm[0];
  let fromArray = []
  if(projectFrom || projectFrom.includes('\\')){
    fromArray = projectFrom.split('\\');
  }
  else if(projectFrom || projectFrom.includes('/')){
    fromArray = projectFrom.split('/');
  }
...
// console.log(shouldBeRelative("C:/Projects/study/src/entities/Articles", "react"));